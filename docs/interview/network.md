...
4. 得到 IP 地址后，进行 TCP 三次握手，<span style="color:orange">参考 15</span>
5. 如果用的是 HTTPS 协议，TCP 三次握手前还有一个 TLS 握手过程，<span style="color:orange">参考 14</span>
6. 成功建立连接后，浏览器发送请求，服务器返回一个 html 作为响应
7. 浏览器解析渲染页面，<span style="color:orange">参考 2</span>
...
4. 根据渲染树来进行布局，这一阶段浏览器就是要搞清楚各个节点在页面中的位置和大小，也叫做回流、重排，<span style="color:orange">参考 3</span>
...
2. 如果想首屏渲染快一点，就把 script 标签放到 body 的末尾；或者给 script 标签加上 defer 或 async 属性，<span style="color:orange">参考 5</span>。
...
1. js 文件名后面加个版本号。
2. 响应头的 Cache-Control 属性设为 no-store，<span style="color:orange">参考 12</span>。
3. 协商缓存，用 Last-Modified 或者 ETag 来控制是否过期，<span style="color:orange">参考 12</span>。
...
1. HTTP 1.1
   1. 新增缓存机制，etag、If-Modefied-Since、If-None-Match 等，<span style="color:orange">参考 12</span>
